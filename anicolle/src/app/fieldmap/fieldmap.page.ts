...
@Component({
  ...
  templateUrl: './fieldmap.page.html',
  styleUrls: ['./fieldmap.page.scss'],
})
export class FieldmapPage {
  ...
  ionViewDidEnter() {
    ...
    if (!this.map) {
      ...
      this.getNearUser(this.currentPosition, 0.1);
    }
  ...
  updateGeoRecord(userId: string, geo: number[]) {
    ...
    const json = {
      ...
      tweet: this.mytweet,
      iconurl: ProfileService.myLocalProfile.iconurl
    };
